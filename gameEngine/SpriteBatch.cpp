...
namespace gameEngine{
	...
	texture(Texture), depth(Depth) {
		topLeft.color = color;
		topLeft.setPosition(destRect.x, destRect.y + destRect[3]);
		topLeft.setUV(uvRect.x, uvRect.y + uvRect[3]);
		...
		topRight.setPosition(destRect.x + destRect.z, destRect.y + destRect[3]);
		topRight.setUV(uvRect.x + uvRect.z, uvRect.y + uvRect[3]);
	}
	...
	void SpriteBatch::createVertexArray(){
		...
		//Tell openGL that we want to use the first attribute array. We only need one array right now since we are only using position.
