...
Gun::Gun(std::string name, int fireRate, int bulletsPerShot, float spread, float bulletDamage, float bulletSpeed, gameEngine::SoundEffect fireEffect) :
_name(name),
_fireRate(fireRate),
...
void Gun::fire(glm::vec2 direction, const glm::vec2 position, std::vector<Bullet>& bullets) {
	...
	std::uniform_real_distribution<float> randRotate(-0.3f, 0.3f);
